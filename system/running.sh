...
echo -e "\e[0;32mSCRIPT VPS khaiVPN\e[0m"
echo "-----------------------------------------------------------"
...
if [ "${status_text}" == "active" ]
then
echo -e " SSH SLOW-DNS            : "$green"running"$NC" ✓"
else
echo -e " SSH SLOW-DNS            : "$red"not running (Error)"$NC" "
fi
...
echo -e "\e[0;34m-----------------------------------------------------------\e[0m"
echo -e ""
echo -e "${red}JIKA TERDAPAT NOT RUNNING, PLEASE REPORT TO ADMIN FOR FIX$NC"
echo -e "${red}SEMOGA DI MURAH KAN REZEKI KITA MASING-MASING..$NC"
